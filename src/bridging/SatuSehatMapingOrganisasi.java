...
public final class SatuSehatMapingOrganisasi extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Data Mapping Organisasi Satu Sehat ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 1, 12))); // NOI18N
        ...
        btnDepartemenRS.setBounds(706, 10, 28, 23);
    ...
    private void btnDepartemenRSActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDepartemenRSActionPerformed
        poli.setSize(886, internalFrame1.getHeight() - 40);
        ...
        if(KodeDepartemen.getText().trim().equals("")||NamaDepartemen.getText().trim().equals("")){
            ...
            if(tbMapping.getSelectedRow()>-1){
                try{
                    ...
                    json = "{" +
                                "\"resourceType\": \"Organization\"," +
                                "\"id\": \""+tbMapping.getValueAt(tbMapping.getSelectedRow(),2).toString()+"\"," +
                                "\"active\": false," +
                                "\"identifier\": [" +
                                    "{" +
                                        "\"use\": \"official\"," +
                    ...
                    System.out.println("Request JSON : "+json);
                    requestEntity = new HttpEntity(json,headers);
                    json=api.getRest().exchange(link+"/Organization/"+tbMapping.getValueAt(tbMapping.getSelectedRow(),2).toString(), HttpMethod.PUT, requestEntity, String.class).getBody();
                    System.out.println("Result JSON : "+json);
                    root = mapper.readTree(json);
                    response = root.path("id");
                    if(!response.asText().equals("")){
                ...
                try{
                    ...
                    if(!response.asText().equals("")){
                        if(Sequel.mengedittf("satu_sehat_mapping_departemen","dep_id=?","dep_id=?,id_organisasi_satusehat=?",3,new String[]{
                                KodeDepartemen.getText(),response.asText(),tbMapping.getValueAt(tbMapping.getSelectedRow(),0).toString()
                            })==true){
                            emptTeks();
                            tampil();
                        }
    ...
    private void tbMappingKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbMappingKeyReleased
        if(tabMode.getRowCount()!=0){
    ...
    }//GEN-LAST:event_tbMappingKeyReleased
    ...
    */
    ...
    private void getData() {
       if(tbMapping.getSelectedRow()!= -1){
           KodeDepartemen.setText(tbMapping.getValueAt(tbMapping.getSelectedRow(),0).toString());
           NamaDepartemen.setText(tbMapping.getValueAt(tbMapping.getSelectedRow(),1).toString());
        }
    }
