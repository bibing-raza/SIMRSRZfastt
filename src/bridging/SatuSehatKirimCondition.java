...
public final class SatuSehatKirimCondition extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Pengiriman Data Condition (Diagnosa ICD-10) Satu Sehat ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 1, 12))); // NOI18N
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        try{
            ...
            for (i = 0; i < tabMode.getRowCount(); i++) {
                htmlContent.append(
                    "<tr class='isi'>"+
                        "<td valign='top'>"+tbCondition.getValueAt(i,1).toString()+"</td>"+
                        "<td valign='top'>"+tbCondition.getValueAt(i,2).toString()+"</td>"+
                        "<td valign='top'>"+tbCondition.getValueAt(i,3).toString()+"</td>"+
                        "<td valign='top'>"+tbCondition.getValueAt(i,4).toString()+"</td>"+
                        "<td valign='top'>"+tbCondition.getValueAt(i,5).toString()+"</td>"+
                        "<td valign='top'>"+tbCondition.getValueAt(i,6).toString()+"</td>"+
                        "<td valign='top'>"+tbCondition.getValueAt(i,7).toString()+"</td>"+
                        "<td valign='top'>"+tbCondition.getValueAt(i,8).toString()+"</td>"+
                        "<td valign='top'>"+tbCondition.getValueAt(i,9).toString()+"</td>"+
                        "<td valign='top'>"+tbCondition.getValueAt(i,10).toString()+"</td>"+
                        "<td valign='top'>"+tbCondition.getValueAt(i,11).toString()+"</td>"+
                        "<td valign='top'>"+tbCondition.getValueAt(i,12).toString()+"</td>"+
                    "</tr>");
            }
            LoadHTML.setText(
                "<html>"+
                ...
                try {
                    ...
                    try {
                        ...
                        json = "{" +
                                    ...
                                    "\"code\": {" +
                                        "\"coding\": [" +
                                            "{" +
                                                ...
                                                "\"code\": \""+tbCondition.getValueAt(i,10).toString()+"\"," +
                                                "\"display\": \""+tbCondition.getValueAt(i,11).toString()+"\"" +
                                            "}" +
                                        "]" +
                                    "}," +
                                    "\"subject\": {" +
                                    ...
                                    "}," +
                                    "\"encounter\": {" +
                                        "\"reference\": \"Encounter/"+tbCondition.getValueAt(i,9).toString()+"\"," +
                                        "\"display\": \"Diagnosa "+tbCondition.getValueAt(i,4).toString()+" selama kunjungan/dirawat dari tanggal "+tbCondition.getValueAt(i,1).toString()+" sampai "+tbCondition.getValueAt(i,8).toString()+"\"" +
                                    "}" +
                                "}";
    ...
    private void ppBersihkanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppBersihkanActionPerformed
        for (i = 0; i < tbCondition.getRowCount(); i++) {
            tbCondition.setValueAt(false, i, 0);
        }
            ...
            if (tbCondition.getValueAt(i, 0).toString().equals("true") 
                ...
                try {
                    ...
                    try {
                        ...
                        json = "{" +
                                    "\"resourceType\": \"Condition\"," +
                                    "\"id\": \""+tbCondition.getValueAt(i,12).toString()+"\"," +
                                    "\"clinicalStatus\": {" +
                                        "\"coding\": [" +
                                            "{" +
                                                "\"system\": \"http://terminology.hl7.org/CodeSystem/condition-clinical\"," +
                                    ...
                                    "}" +
                                "}";
                        System.out.println("URL : " + link + "/Condition/" + tbCondition.getValueAt(i, 12).toString());
                        System.out.println("Request JSON : " + json);
                        requestEntity = new HttpEntity(json, headers);
                        json = api.getRest().exchange(link + "/Condition/" + tbCondition.getValueAt(i, 12).toString(), HttpMethod.PUT, requestEntity, String.class).getBody();
                        System.out.println("Result JSON : " + json);
                    } catch (Exception e) {
                        System.out.println("Notifikasi Bridging : " + e);
                    }
        ...
        try {
            if (cmbData.getSelectedIndex() == 0) {
                ps = koneksi.prepareStatement(
                        ...
                        + "concat(rp.tgl_registrasi,' ',ADDTIME(rp.jam_reg,'02:00:00')) pulang, se.id_encounter, dp.kd_penyakit, py.nm_penyakit, "
                        + "ifnull(sc.id_condition,'') id_condition FROM reg_periksa rp INNER JOIN pasien p ON rp.no_rkm_medis = p.no_rkm_medis "
            ...
            if (cmbData.getSelectedIndex() == 0) {
                ps = koneksi.prepareStatement(
                        "SELECT rp.tgl_registrasi, rp.jam_reg, rp.no_rawat, rp.no_rkm_medis, p.nm_pasien, p.no_ktp, rp.stts, rp.status_lanjut, concat(ki.tgl_keluar,' ',ki.jam_keluar) pulang, "
                        + "se.id_encounter, dp.kd_penyakit, py.nm_penyakit, ifnull(sc.id_condition,'') id_condition FROM reg_periksa rp INNER JOIN pasien p ON rp.no_rkm_medis = p.no_rkm_medis "
