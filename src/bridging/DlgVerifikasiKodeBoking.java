...
public class DlgVerifikasiKodeBoking extends javax.swing.JDialog {
    ...
    private int x = 0, cekDaftar = 0, cekSEP = 0, cekBoking = 0, cekKonsul = 0;
    ...
    private String tglCekbpjs = "", cekStatusCetak = "", normbpjs = "", kdpolibpjs = "", kdpenjabbpjs = "", kddokterbpjs = "",
            ...
            utc = "", URL = "", requestJson, respons = "", LokasiLaka = "", cekSipo = "", norawat = "", statusSEP = "", wktSimpan = "";
    ...
    private boolean simpanRegistrasiBPJS() {
        ...
        if (Sequel.menyimpantf2("reg_periksa", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "No.Rawat", 19,
            ...
            Sequel.mengedit("reg_konsul_internal", "waktu_simpan='" + wktSimpan + "'", "no_rawat='" + norwBokingBPJS.getText() + "',jalur_registrasi='tpprj'");
...
//sampai sini -------------
    ...
    private void simpanVerifikasi() {
        ...
        cekKonsul = 0;
        wktSimpan = "";
        cekBoking = Sequel.cariInteger("select count(-1) from booking_registrasi where kd_booking ='" + kdbokingbpjs.getText() + "'");
        cekKonsul = Sequel.cariInteger("select count(-1) from reg_konsul_internal where no_rawat ='" + kdbokingbpjs.getText() + "'");
        wktSimpan = Sequel.cariIsi("select waktu_simpan from reg_konsul_internal where no_rawat ='" + kdbokingbpjs.getText() + "'");
        cekKodeBokingBPJS();
        emptTeksBokingBPJS();

        if (cekBoking > 0 && cekKonsul == 0) {
            ...
            if (tglCekbpjs.equals(Valid.SetTgl(tglPeriksaBPJS.getSelectedItem() + ""))) {
                ...
                if (kdpenjabbpjs.equals("B01") || (kdpenjabbpjs.equals("A03"))) {
                    if (cekDaftar > 0 && cekStatusCetak.equals("SUDAH") || Sequel.cariInteger("select count(-1) from reg_periksa where no_rawat='"
                            + Sequel.cariIsi("select no_rawat from booking_registrasi where kd_booking ='" + kdbokingbpjs.getText() + "'") + "'") > 0) {
                    ...
                    } else if (cekDaftar == 0 && cekStatusCetak.equals("GAGAL") || Sequel.cariInteger("select count(-1) from booking_registrasi where "
                            + "kd_booking ='" + kdbokingbpjs.getText() + "' and status_booking='Batal'") > 0) {
        ...
        } else if (cekBoking > 0 && cekKonsul > 0) {
            cekPasienBokingBPJS();
            umurPasienBPJS();
            cekStatusCetak = Sequel.cariIsi("select status_booking from booking_registrasi where kd_booking='" + kdbokingbpjs.getText() + "'");

            if (kdpenjabbpjs.equals("B01") || (kdpenjabbpjs.equals("A03"))) {
                if (cekStatusCetak.equals("Terdaftar")) {
                    JOptionPane.showMessageDialog(null, "Pasien dg. kode booking " + kdbokingbpjs.getText() + " telah terdaftar pada hari ini & sudah mencetak SEP...!!!");
                    BtnKeluarActionPerformed(null);
                } else if (cekStatusCetak.equals("Menunggu")) {
                    simpanRegKonsul();
                } else if (cekStatusCetak.equals("Batal")) {
                    JOptionPane.showMessageDialog(null, "Pasien dg. kode booking " + kdbokingbpjs.getText() + " telah membatalkan proses bookingnya, silakan mendaftar lagi keloket pendaftaran..!!!");
                    BtnKeluarActionPerformed(null);
                }
            } else {
    ...
    private void hapusReg() {
        ...
        Sequel.meghapus("reg_konsul_internal", "no_rawat", norawat);
