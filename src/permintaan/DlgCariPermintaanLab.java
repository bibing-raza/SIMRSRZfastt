...
public class DlgCariPermintaanLab extends javax.swing.JDialog {
    ...
    private int i, diagnosa_cek1 = 0, diagnosa_cek2 = 0;
    private PreparedStatement ps, ps2, ps3;
    private ResultSet rs, rs2, rs3;
    private String tglsampel = "", tglhasil = "", norm = "", kamar = "", namakamar = "", diagnosa = "",
            la = "", ld = "", pa = "", pd = "", diagnosa_ok1 = "", diagnosa_ok2 = "", norawat = "", cekdokter1 = "",
            cekdokter2 = "", kodenya = "", jnsrwt = "", norw = "", cekUnit = "", nrawat = "", jrawat = "", polinya = "";
    ...
    private void initComponents() {
        ...
        MnBarcodePermintaan.setForeground(new java.awt.Color(0, 0, 0));
        ...
        MnBarcodePermintaan1.setForeground(new java.awt.Color(0, 0, 0));
        ...
        internalFrame5.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Update Waktu Pengambilan Sampel ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 1, 12), new java.awt.Color(0, 0, 0))); // NOI18N
        ...
        TanggalPulang.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "27-01-2021 11:19:59" }));
        ...
        BtnHasil.setMnemonic('V');
        BtnHasil.setText("Verifikasi");
        BtnHasil.setToolTipText("Alt+V");
        ...
        BtnSampel.setForeground(new java.awt.Color(0, 0, 0));
    ...
    if (TabRawat.getSelectedIndex() == 0) {
        if (tbDokter.getSelectedRow() != -1) {
            if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim().equals("")) {
                JOptionPane.showMessageDialog(null, "Silahkan pilih dulu salah satu datanya pada tabel dengan mengklik nama pasien...!!!!");
            } else {
...
private void tbDokterKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbDokterKeyPressed
   if(tabMode.getRowCount()!=0){
    ...
    private void BtnHasilActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHasilActionPerformed
        ...
        diagnosa_cek1 = Sequel.cariInteger("select count(1) cek from pemeriksaan_ralan where no_rawat='" + norawat + "'");
        diagnosa_cek2 = Sequel.cariInteger("select count(1) cek from kamar_inap where no_rawat='" + norawat + "'");
        cekdokter1 = Sequel.cariIsi("select kd_dokter from reg_periksa where no_rawat='" + norawat + "' and status_lanjut='Ralan'");
        cekdokter2 = Sequel.cariIsi("select kd_dokter from dpjp_ranap where no_rawat='" + norawat + "'");
                
        if (diagnosa_cek1 == 0) {
            diagnosa_ok1 = "-";
        } else if (diagnosa_cek2 == 0) {
            diagnosa_ok2 = "-";
        } else if (diagnosa_cek1 >= 1) {
            diagnosa_ok1 = Sequel.cariIsi("select diagnosa from pemeriksaan_ralan where no_rawat='" + norawat + "'");
        } else if (diagnosa_cek2 >= 1) {
            diagnosa_ok2 = Sequel.cariIsi("select diagnosa_awal from kamar_inap where no_rawat='" + norawat + "'");
        }
...
//                        dlgro.emptTeks();
//                        dlgro.isCek();
//                        dlgro.KodePerujuk.setText(cekdokter2);
//                        dlgro.setNoRm(tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString(), "Ranap", "-", diagnosa_ok2,"nama bangsalnya belum disetting");                        
//                        dlgro.setOrder(tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString());
//                        dlgro.setVisible(true);
//                        this.setCursor(Cursor.getDefaultCursor());
//                    } else {

                    if (jnsrwt.equals("-") && norw.equals("-")) {
                        nrawat = Sequel.cariIsi("select no_rawat from permintaan_lab where noorder='" + tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString() + "'");
                        jrawat = Sequel.cariIsi("select status_lanjut from reg_periksa where no_rawat='" + nrawat + "'");
                        polinya = Sequel.cariIsi("select pl.nm_poli from poliklinik pl inner join reg_periksa rp on rp.kd_poli=pl.kd_poli where rp.no_rawat='" + nrawat + "'");

                        if (jrawat.equals("Ralan")) {
                            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                            DlgPeriksaLaboratorium dlgro = new DlgPeriksaLaboratorium(null, false);
                            dlgro.setSize(internalFrame1.getWidth() - 40, internalFrame1.getHeight() - 40);
                            dlgro.setLocationRelativeTo(internalFrame1);
                            dlgro.emptTeks();
                            dlgro.isCek();
                            dlgro.KodePerujuk.setText(cekdokter1);
                            dlgro.setNoRm(tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString(), jrawat, diagnosa_ok1, "-", polinya);
                            dlgro.setOrder(tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString());
                            dlgro.setVisible(true);
                            this.setCursor(Cursor.getDefaultCursor());
                        }
                        ...
                        dlgro.isCek();
                        dlgro.KodePerujuk.setText(cekdokter1);
                        dlgro.setNoRm(tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString(), "Ralan", diagnosa_ok1, "-", Sequel.cariIsi("select nm_poli from poliklinik where kd_poli='" + kodenya + "'"));
                        dlgro.setOrder(tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString());
                        dlgro.setVisible(true);
                        this.setCursor(Cursor.getDefaultCursor());
                    }
    ...
    private void MnBarcodePermintaanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnBarcodePermintaanActionPerformed
        if (tbDokter.getSelectedRow() != -1) {
            if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim().equals("")) {
                ...
                Valid.MyReport("rptBarcodePermintaanLab.jasper", "report", "::[ Barcode No.Permintaan Lab ]::",
    ...
    private void MnBarcodePermintaan1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnBarcodePermintaan1ActionPerformed
        if (tbDokter.getSelectedRow() != -1) {
            if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim().equals("")) {
                ...
                param.put("kontakrs", akses.getkontakrs());
                param.put("emailrs", akses.getemailrs());
                Valid.MyReport("rptBarcodePermintaanLab2.jasper", "report", "::[ Barcode No.Permintaan Lab ]::",
                        "select noorder from permintaan_lab where no_rawat='" + tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString() + "'", param);
                this.setCursor(Cursor.getDefaultCursor());
            }
        } else {
    ...
    */
    ...
    private void getData() {
        ...
        if(tbDokter.getSelectedRow()!= -1){
            ...
            norawat = tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString();
        }
    }
    
    public void isCek(String jns_rawat, String norwt){
        jnsrwt = jns_rawat;
        norw = norwt;
        ...
        if (jnsrwt.equals("Ralan")) {
            kodenya = Sequel.cariIsi("select kd_poli from reg_periksa where no_rawat='" + norw + "'");
        } else if (jnsrwt.equals("Ranap")) {
            //masih nunggu implementasi penerapan dulu, nanti dipikirkan lagi
        } else {
            kodenya = "";
        }
    }
    ...
    private void cetakLapRekap() {
        if (TabRawat.getSelectedIndex() == 0) {
            if (tabMode.getRowCount() == 0) {
                ...
                Valid.MyReport("rptLapPermintaanLab.jasper", "report", "::[ Data Permintaan Laboratorium ]::",
            ...
            if (tabMode2.getRowCount() == 0) {
                ...
                param.put("emailrs", akses.getemailrs());
                param.put("logo", Sequel.cariGambar("select logo from setting"));
                Valid.MyReport("rptLapPermintaanLab2.jasper", "report", "::[ Data Detail Permintaan Laboratorium ]::",
                        "select no, temp1, temp2, temp3, temp4, temp5, temp6, temp7, temp8, temp9, temp10, temp11, temp12, temp13, temp14, temp14, temp15, temp16 from temporary_permintaan_lab order by no asc", param);
            }
        }
    }
    
    private void cetakPerminLab() {
                if (tbDokter.getSelectedRow() != -1) {
            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim().equals("")) {
                JOptionPane.showMessageDialog(null, "Silahkan pilih dulu salah satu datanya pada tabel dengan mengklik nama pasien...!!!!");
                ...
                if (!kamar.equals("")) {
                    ...
                    diagnosa = Sequel.cariIsi("select ifnull(diagnosa_awal,'-') diagnos from kamar_inap where no_rawat=?", tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString());                    
                    ...
                    diagnosa = Sequel.cariIsi("select IFNULL(diagnosa,'-') diag_resum from pemeriksaan_ralan where no_rawat=?", tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString());
                }
                ...
                param.put("alamat", Sequel.cariIsi("select concat(pasien.alamat,', ',kelurahan.nm_kel,', ',kecamatan.nm_kec,', ',kabupaten.nm_kab) as alamat from pasien inner join "
                        +"kelurahan inner join kecamatan inner join kabupaten on pasien.kd_kel=kelurahan.kd_kel and pasien.kd_kec=kecamatan.kd_kec and "
                ...
                param.put("emailrs", akses.getemailrs());
                param.put("logo", Sequel.cariGambar("select logo from setting"));
                Valid.MyReport("rptPermintaanLab.jasper", "report", "::[ Permintaan Laboratorium ]::",
                        "select no, temp1, temp2, temp3, temp4, temp5, temp6, temp7, temp8, temp9, temp10, temp11, temp12, temp13, temp14, temp14, temp15, temp16 from temporary_permintaan_lab order by no asc", param);
            }
            this.setCursor(Cursor.getDefaultCursor());
        } else {
