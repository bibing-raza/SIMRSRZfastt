...
public class DlgDpjp extends javax.swing.JDialog {
    ...
    public DlgDpjp(java.awt.Frame parent, boolean modal) {
        ...
        String[] row = {"Tgl. Rawat", "No. Rawat", "No. RM", "Nama Pasien", "Kode Dokter", "Nama Dokter", "tgl_reg"};
        ...
        tbDPJP.getColumnModel().getColumn(2).setCellRenderer(centerRenderer);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (TNoRw.getText().equals("")) {
            ...
            if (akses.getPasteData().equals("perubahan dpjp")) {
                JOptionPane.showMessageDialog(null, "Perubahan DPJP telah dilakukan, data sudah tersimpan..!!");
                BtnBatalActionPerformed(null);
            } else if (Sequel.cariInteger("select count(-1) from dpjp_ranap where no_rawat='" + TNoRw.getText() + "'") > 0) {
    ...
    public void tampil() {
        ...
        try {
            ps = koneksi.prepareStatement("SELECT date_format(rp.tgl_registrasi,'%d/%m/%Y') tglreg, dr.no_rawat, rp.no_rkm_medis, "
                    ...
                    + "d.kd_dokter like ? or "
                    + "d.nm_dokter like ? or "
                    + "rp.no_rkm_medis like ? order by rp.tgl_registrasi");
            try {                
