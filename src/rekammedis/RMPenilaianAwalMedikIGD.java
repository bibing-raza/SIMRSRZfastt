...
public final class RMPenilaianAwalMedikIGD extends javax.swing.JDialog {
    ...
    public RMPenilaianAwalMedikIGD(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{
            ...
            "diameter_kiri", "gangguan_jalan_nafas", "nadi_2", "akral_2", "cekjamkeluar", "tglkeluar", "jamkeluar", "id_file_nm_penerima_edukasi"
        }) {
    ...
    private void initComponents() {
        ...
        internalFrame7.add(panelGlass1, java.awt.BorderLayout.CENTER);
        ...
        jLabel112.setText("Pilih Rekam Medis :");
        jLabel112.setName("jLabel112"); // NOI18N
        panelisi3.add(jLabel112);
        jLabel112.setBounds(0, 10, 120, 23);
        ...
        FormInput.setPreferredSize(new java.awt.Dimension(870, 2278));
        ...
        TglAsesmen.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "26-06-2026 23:01:37" }));
        ...
        ChkBendaAsing.setBounds(20, 400, 90, 23);
        ...
        scrollPane13.setBorder(javax.swing.BorderFactory.createTitledBorder(new javax.swing.border.LineBorder(new java.awt.Color(0, 0, 0), 1, true), " TERINDIKASI ALERGI ", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 1, 12))); // NOI18N
        ...
        scrollPane13.setViewportView(TindikasiAlergi);

        FormInput.add(scrollPane13);
        scrollPane13.setBounds(900, 10, 547, 170);
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        if (tbPenilaian.getSelectedRow() > -1) {
            ...
            if (cmbPilihCetak.getSelectedIndex() == 0) {
                String isiDpjp = "", isiPerawat = "", isiPetugas = "", isiDokter = "", tgl = "", jam = "";
                tgl = Sequel.cariIsi("select date_format(waktu_simpan,'%d/%m/%Y') from penilaian_awal_medis_igd where "
                        + "no_rawat='" + tbPenilaian.getValueAt(tbPenilaian.getSelectedRow(), 0).toString() + "'");
                jam = Sequel.cariIsi("select time(waktu_simpan) from penilaian_awal_medis_igd where "
                        + "no_rawat='" + tbPenilaian.getValueAt(tbPenilaian.getSelectedRow(), 0).toString() + "'");
                ...
                if (nmdpjp.getText().equals("") || nmdpjp.getText().equals("-") || nmdpjp.getText().equals("--")) {
                    JOptionPane.showMessageDialog(rootPane, "Nama DPJP asesmen medik gawat darurat harus diisi dulu,..");
                } else if (nmpetugas.getText().equals("") || nmpetugas.getText().equals("-") || nmpetugas.getText().equals("--")) {
                    JOptionPane.showMessageDialog(rootPane, "Nama pemberi edukasi/informasi asesmen medik gawat darurat harus diisi dulu,..");
                } else if (nmdokter.getText().equals("") || nmdokter.getText().equals("-") || nmdokter.getText().equals("--")) {
                    JOptionPane.showMessageDialog(rootPane, "Nama dokter pemberi edukasi/informasi asesmen medik gawat darurat harus diisi dulu,..");
                } else {
                    isiDpjp = Sequel.cariIsi("select replace(kalimat_qrcode,kalimat_qrcode,"
                            + "'" + Valid.kalimatQRcode(Sequel.cariIsi("select jenis_dokumen from kalimat_tte where kode='001'"),
                                    "Assesmen Medik Gawat Darurat", nmdpjp.getText() + " (DPJP)", tgl, jam) + "') from kalimat_tte where kode='001'");                    
                    isiPetugas = Sequel.cariIsi("select replace(kalimat_qrcode,kalimat_qrcode,"
                            + "'" + Valid.kalimatQRcode(Sequel.cariIsi("select jenis_dokumen from kalimat_tte where kode='001'"),
                                    "Assesmen Medik Gawat Darurat", nmpetugas.getText() + " (Petugas Pemberi Edukasi)", tgl, jam) + "') from kalimat_tte where kode='001'");
                    ...
                    if (NmPerawat.getText().equals("") || NmPerawat.getText().equals("-") || NmPerawat.getText().equals("--")) {
                        ...
                        isiPerawat = Sequel.cariIsi("select replace(kalimat_qrcode,kalimat_qrcode,"
                                + "'" + Valid.kalimatQRcode(Sequel.cariIsi("select jenis_dokumen from kalimat_tte where kode='001'"),
                                        "Assesmen Medik Gawat Darurat", NmPerawat.getText() + " (Perawat)", tgl, jam) + "') from kalimat_tte where kode='001'");
                    ...
                    Valid.MyReport("rptCetakPenilaianAwalMedisIGDQr.jasper", "report", "::[ Laporan Penilaian Awal Medis IGD hal. 1 ]::",
                    ...
                    Valid.MyReport("rptCetakPenilaianAwalMedisIGD1Qr.jasper", "report", "::[ Laporan Penilaian Awal Medis IGD hal. 2 ]::",
                            "SELECT p.no_rkm_medis, p.nm_pasien, date_format(p.tgl_lahir,'%d-%m-%Y') tgllahir, "
                            + "date_format(pa.tanggal,'Tanggal : %d-%m-%Y    Pukul : %H:%i') mulai_penanganan, if(pa.cervival='ya','V','') cer, "
                            + "if(pa.rjp='ya','V','') rjp, if(pa.defribilasi='ya','V','') def, if(pa.intubasi='ya','V','') intu, if(pa.vtp='ya','V','') vtp, if(pa.dekompresi='ya','V','') dek, "
                            + "if(pa.balut='ya','V','') bal, if(pa.kateter='ya','V','') kat, if(pa.ngt='ya','V','') ngt, if(pa.infus='ya','V','') infs, if(pa.obat='ya','V','') obt, pa.ket_obat, "
                    ...
                    Valid.MyReport("rptCetakPenilaianAwalMedisIGD2Qr.jasper", "report", "::[ Laporan Penilaian Awal Medis IGD hal. 2 ]::",
                            "SELECT p.no_rkm_medis, p.nm_pasien, date_format(p.tgl_lahir,'%d-%m-%Y') tgllahir, "
                            + "date_format(pa.tanggal,'Tanggal : %d-%m-%Y    Pukul : %H:%i') mulai_penanganan, if(pa.cervival='ya','V','') cer, "
                            + "if(pa.rjp='ya','V','') rjp, if(pa.defribilasi='ya','V','') def, if(pa.intubasi='ya','V','') intu, if(pa.vtp='ya','V','') vtp, if(pa.dekompresi='ya','V','') dek, "
                            + "if(pa.balut='ya','V','') bal, if(pa.kateter='ya','V','') kat, if(pa.ngt='ya','V','') ngt, if(pa.infus='ya','V','') infs, if(pa.obat='ya','V','') obt, pa.ket_obat, "
                    ...
                    if (inet.isReachable(100)) {
                        if (idFileTtd.equals("")) {
                            ...
                            if (Sequel.hapusFileTTD(idFileTtd) == true) {
                                Sequel.mengedit("penilaian_awal_medis_igd", "no_rawat='" + tbPenilaian.getValueAt(tbPenilaian.getSelectedRow(), 0).toString() + "'", 
    ...
    private void MnBikinQrCodeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnBikinQrCodeActionPerformed
        if (tbPenilaian.getSelectedRow() > -1) {
            ((RMPenilaianAwalMedikIGD.Painter) gambarQR).setImage("");
            if (Sequel.cariInteger("select count(-1) from master_nomor_dokumen_erm where nm_dokumen like '%ASESMEN MEDIK GAWAT DARURAT%'") > 0) {
                bikinQR();
            } else {
        ...
        if (cmbRM.getSelectedIndex() != 0) {
            ...
            try {
                ...
                for (int I = 0; hasil.next(); I++) {
                    ...
                    ((RMPenilaianAwalMedikIGD.Painter) gambarQR).setImageIcon(new javax.swing.ImageIcon(
    ...
    */
    ...
    private void getData() {
        ...
        if (tbPenilaian.getSelectedRow() != -1) {
            ...
            cmbMnt1.setSelectedItem(tbPenilaian.getValueAt(tbPenilaian.getSelectedRow(), 130).toString().substring(3, 5));
            cmbDtk1.setSelectedItem(tbPenilaian.getValueAt(tbPenilaian.getSelectedRow(), 130).toString().substring(6, 8));
            idFileTtd = tbPenilaian.getValueAt(tbPenilaian.getSelectedRow(), 131).toString();
            hitungSkorDowne();
            dataCek();
            cekAlergi();
            tampilTTD();
    ...
    public void isCek() {
        ...
        BtnDpjp.setEnabled(akses.getpic_igd());
    ...
    private void ganti() {
        ...
        if (Sequel.mengedittf("penilaian_awal_medis_igd", "no_rawat=?", "no_rawat=?, tanggal=?, cervival=?, rjp=?, defribilasi=?, intubasi=?, vtp=?, dekompresi=?,"
            ...
            Sequel.SimpanHistoriRekamMedis(TNoRw.getText(), "Assesmen Medik Gawat Darurat", "Ganti");
    ...
    private void simpan() {
        ...
        if (Sequel.menyimpantf("penilaian_awal_medis_igd", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,"
            ...
            Sequel.SimpanHistoriRekamMedis(TNoRw.getText(), "Assesmen Medik Gawat Darurat", "Simpan");
    ...
    private void cekAlergi() {
        ...
        try {
            ...
            ps16 = koneksi.prepareStatement(sb.toString());
            try {
                ps16.setString(1, TNoRM.getText());
                rs16 = ps16.executeQuery();
                if (rs16.next()) {
                    if (!rs16.getString("alergiObat").equals("")) {
                        alerObat = rs16.getString("alergiObat") + "\n";
                    } else {
                        alerObat = "";
                    }

                    if (!rs16.getString("alergiMakan").equals("")) {
                        alerMak = rs16.getString("alergiMakan") + "\n";
                    } else {
                        alerMak = "";
                    }

                    if (!rs16.getString("alergiLain").equals("")) {
                        alerLain = rs16.getString("alergiLain") + "\n";
                    } else {
                        alerLain = "";
                    }

                    if (!rs16.getString("alergiDiberitau").equals("")) {
                        alerDiberi = rs16.getString("alergiDiberitau");
                    } else {
                        alerDiberi = "";
                    }
    ...
    private void tampilTTD() {
        try {
            ...
            htmlContent.append(
                    "<tr class='isi'>"
                    + "<td valign='middle' align='center'><img src='" + gambar + "' width='160' height='160' alt='TTD Penerima Edukasi'><br>(" + nmpenerima.getText() + ")<br></td>"
                    + "</tr>"
            );
    ...
    private void bikinQR() {        
        ...
        Valid.cetakQrTte(URL, Sequel.cariFolderTte(), "QRTte.jpg", "select logo from setting");
        Sequel.menyimpanQrTte("parameter_ttd_rme", "'" + idParameterTtd + "','" + usernya + "','" + pwdnya + "','" + TNoRw.getText() + "','" + TNoRM.getText() + "','"
                + Sequel.cariIsi("select kode_erm from master_nomor_dokumen_erm where nm_dokumen like '%ASESMEN MEDIK GAWAT DARURAT%'") + "',"
                + "'" + Sequel.cariIsi("select now()") + "'", "file QRCode URL Ttd", Sequel.cariFolderPrintTte());
