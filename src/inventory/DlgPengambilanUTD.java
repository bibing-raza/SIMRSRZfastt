...
public class DlgPengambilanUTD extends javax.swing.JDialog {
    ...
    private double stok_asal, stok_tujuan, subtotal, y, stokbarang2;
...
//        } 
            ...
            i = JOptionPane.showConfirmDialog(rootPane, "Data mau disimpan..??", "Konfirmasi", JOptionPane.YES_NO_OPTION);
            if (i == JOptionPane.YES_OPTION) {
                Sequel.AutoComitFalse();
                for (i = 0; i < tbDokter.getRowCount(); i++) {
                    try {
                        if (Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) > 0) {
                            if ((tbDokter.getValueAt(i, 0).toString()) == null || (tbDokter.getValueAt(i, 0).toString()).equals("")) {
                                y = 0;
                            } else {
                                y = Double.parseDouble(tbDokter.getValueAt(i, 0).toString());
                            }
                            ...
                            if (stokbarang2 < y || TStok.getText().equals("")) {
                                ...
                                tbDokter.setValueAt("", tbDokter.getSelectedRow(), 1);
                                ppBersihkanActionPerformed(evt);
                            } else {
                                if (Sequel.menyimpantf("utd_pengambilan_medis", "'" + tbDokter.getValueAt(i, 1).toString() + "','" + tbDokter.getValueAt(i, 0).toString() + "','"
                                    ...
                                    Trackobat.catatRiwayat2(tbDokter.getValueAt(i, 1).toString(), 0, Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()), "Pengambilan Medis", akses.getkode(), kddari.getText(), "Simpan");
            ...
            //JOptionPane.showMessageDialog(null,"Maaf, gagal menyimpan data. Kemungkinan ada data yang sama dimasukkan sebelumnya?\nKapasitas belum dimasukkan...!BAWAH");
                        JOptionPane.showMessageDialog(null, "Maaf, gagal menyimpan data. Kemungkinan ada query yang salah, hubungi admin...!");
                    }
