...
public class DlgMutasiBarang extends javax.swing.JDialog {
    ...
    private int jml = 0, i = 0, row = 0, index = 0, pilihan = 0, count = 0;
    private String[] kodebarang, namabarang, jumlah, satuan, stokasal, stoktujuan, sisastokasal, sisastoktujuan;
    ...
    public DlgMutasiBarang(java.awt.Frame parent, boolean modal) {
        ...
        Object[] judul = {"Jml", "Kode Barang", "Nama Barang", "Satuan", "Stok Asal", "Stok Tujuan", "Sisa Stok Asal", "Sisa Stok Tujuan"};
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (nmdari.getText().trim().equals("") || kddari.getText().trim().equals("")) {
            ...
            if (i == JOptionPane.YES_OPTION) {
                ...
                for (i = 0; i < tbDokter.getRowCount(); i++) {
                    try {
                        if (Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) > 0) {
                            ...
                            isStok(tbDokter.getValueAt(i, 1).toString());
                            if (stokbarang2 < Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) || stokbarang2 == 0) {
                                count++;
                            }
                        }
                    } catch (Exception e) {
                ...
                if (count > 0) {
                    JOptionPane.showMessageDialog(rootPane, "Maaf ada barang yang stoknya tidak mencukupi..!!");
                } else {
                    for (i = 0; i < tbDokter.getRowCount(); i++) {
                        try {
                            if (Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) > 0) {
                                if (Sequel.menyimpantf("mutasibarang", "'" + tbDokter.getValueAt(i, 1).toString() + "','" + Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) + "','" + kddari.getText() + "',"
                                    ...
                                    Trackobat.catatRiwayat2(tbDokter.getValueAt(i, 1).toString(), 0, Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()), "Mutasi", akses.getkode(), kddari.getText(), "Simpan");
                                    ...
                                    Trackobat.catatRiwayat2(tbDokter.getValueAt(i, 1).toString(), Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()), 0, "Mutasi", akses.getkode(), kdke.getText(), "Simpan");
...
private void ppBersihkanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppBersihkanActionPerformed
                ...
                if (tbDokter.getSelectedColumn() == 1) {
                    try {
                        if (Valid.SetAngka(tabMode.getValueAt(tbDokter.getSelectedRow(), 0).toString()) > 0) {
                            ...
                            tbDokter.setValueAt(Double.parseDouble(tabMode.getValueAt(tbDokter.getSelectedRow(), 4).toString()) - Double.parseDouble(tabMode.getValueAt(tbDokter.getSelectedRow(), 0).toString()), tbDokter.getSelectedRow(), 6);
                            tbDokter.setValueAt(Double.parseDouble(tabMode.getValueAt(tbDokter.getSelectedRow(), 5).toString()) + Double.parseDouble(tabMode.getValueAt(tbDokter.getSelectedRow(), 0).toString()), tbDokter.getSelectedRow(), 7);
                        }
                    } catch (Exception e) {
    ...
    private void tampil() {
        ...
        sisastokasal = new String[jml];
        sisastoktujuan = new String[jml];
        index = 0;
        for (i = 0; i < row; i++) {
            try {
                if (Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) > 0) {
                    ...
                    sisastokasal[index] = tbDokter.getValueAt(i, 6).toString();
                    sisastoktujuan[index] = tbDokter.getValueAt(i, 7).toString();
                    index++;
                }
            } catch (Exception e) {
            }
        ...
        for (i = 0; i < jml; i++) {
            tabMode.addRow(new Object[]{jumlah[i], kodebarang[i], namabarang[i], satuan[i], stokasal[i], stoktujuan[i], sisastokasal[i], sisastoktujuan[i]});
        }
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                tabMode.addRow(new Object[]{"", rs.getString(1), rs.getString(2), rs.getString(3), 0, 0, 0, 0});
            }
        } catch (Exception e) {
